...
func openDevSystem(config Config) (ifce *TunVInterface, err error) {
...
func openDevTunTapOSX(config Config) (ifce *TunVInterface, err error) {
	var fd int
	var socketFD int
	...
	return &TunVInterface{
		...
		RWC:        os.NewFile(uintptr(fd), fmt.Sprintf("tun-%s", config.Name)),
		Name:       config.Name,
	}, nil
}
